...
int main(void)
{
	...
	// casting to unsigned works if high & low are signed integers but, if they are both unsigned, then we are back to the overflow problem when low+high > UINT_MAX
	mid = ((unsigned int)high + (unsigned int)low) /2;
	...
	*/
	...
	printf("sizeof(int)  =  %d bytes\t\tSize of int is platform dependent.\n", sizeof(int));
	printf("INT_MAX      =  %d\n", INT_MAX);
	printf("INT_MIN      = %d\n", INT_MIN);
	//printf("UINT_MAX     =  %u\n", UINT_MAX);
	printf("\n");
	...
	printf("Add positive or negative values to a short integer\n");
	printf("End program with Ctrl-C\n");
	printf("\t\t short\tbinary value\n");

	while (true) { // prompt until user kills program with Ctrl-C
		...
		/*
		scanf() returns the number of arguments of data type(s) successfuly read [ 0 = false] ; 
		...
		*/
		...
		if (scanf("%hd", &amount)) { // if a number was entered
			sum += amount;
		}
